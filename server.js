class MakeFeedFile {
    ...
    get getNewsFeedTitles() {
        return this.getFeeds();
    }

    getFeeds() {
    
        console.log("todaysFeedJsonFile name: ",this.todaysJsonFeedTitlesFile());

        let todaysFeedTitles = [];

        // check if file exists for today
        this.fs.readFile(this.todaysJsonFeedTitlesFile(), 'utf8', function (err,data) {
            if (err) { // If no file, create it!
                // @TODO make file function???
                return console.log(err);
            }

            // split titles csv into an array
            let titlesArray = data.split("\n");
            let tempWordsArray = [];

            let wordsArray = titlesArray.map((titleLine) => {
                tempWordsArray.push(...titleLine.split(" "));

                 // Set makes a set that has unique chars, the [...] new array with that set
                 return [...new Set(tempWordsArray)];

                 // remove special characters like ? - ! and de-capitalise 
                // @TODO not MVP, do phase two
                //  return tempWordsArray2.map((word) => {
                //     // remove characters 
                //     return this.curateWordsFromTitles(word);
                //  })
                //  tempWordsArray = new Set(tempWordsArray);
            })
...
// split all titles into words, for "Lorem ipsum dolor sit amet" is ["Lorem", "ipsum", "dolor", "sit", "amet"]

// filter the array to only have unique words

// maybe filter ALL CAPS

// create file with todays date in it like "feed-titles_07-08-2020.json"

// write the huge array of words 